...
func (persist *Persist) ListUser(search string, page int) ([]bean.Users, error) {
	...
	_0ffset := page * config.Server.ViewLimit

	var err error

	if len(search) == 0 {
		err = persist.db.Table("users").Select("*").Limit(config.Server.ViewLimit).Offset(_0ffset).Find(&users).Error
		return users, err
	}

	err = persist.db.Table("users").Where("user_id like ?", "%"+search+"%").Select("*").Limit(config.Server.ViewLimit).Offset(_0ffset).Find(&users).Error
